        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs space-y-4">
            ...
            <div className="space-y-3">
              ...
              {/* Quick switch to another existing school in master data if available */}
              {schools.filter((s) => s.id !== activeSchool.id).length > 0 && (
                <div className="pt-2 border-t border-slate-100 flex items-center gap-2 text-xs">
                  <span className="text-slate-500 font-medium shrink-0">Pilih dari Master:</span>
                  <select
                    id="select-active-school-master"
                    value={activeSchool.id}
                    onChange={(e) => {
                      if (e.target.value && onSelectSchool) {
                        onSelectSchool(e.target.value);
                      }
                    }}
                    className="w-full text-xs px-2.5 py-1.5 rounded-lg border border-slate-300 bg-slate-50 text-slate-800 font-medium"
                  >
                    <option value={activeSchool.id}>{activeSchool.name} (Sedang Digunakan)</option>
                    {schools
